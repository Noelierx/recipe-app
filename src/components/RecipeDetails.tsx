...
const RecipeDetails: React.FC = () => {
  ...
  const adjustedMainIngredients = adjustIngredients(
    recipe.recipe_ingredients,
    recipe.servings,
    servings,
  );

  const adjustedSubRecipes =
    recipe.sub_recipes?.map((subRecipe) => ({
      ...subRecipe,
      ingredients: adjustIngredients(
    ...
    <div className="max-w-7xl mx-auto px-4">
      ...
      <div className="flex flex-col md:flex-row">
                ...
                <div key={subIndex} className="mb-4">
                  ...
                  <ul className="list-disc pl-5">
                    {subRecipe.ingredients.map((ing, ingIndex) => (
                      <li key={ingIndex}>
                        {ing.ingredient.name}: {ing.amount.toFixed(2)}{" "}
                        {ing.unit}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="w-full md:w-1/2">
          ...
          {hasSubRecipes &&
            adjustedSubRecipes.map((subRecipe, index) => (
          ...
          {hasSubRecipes && (
            <h3 className="text-xl font-semibold mb-2">La suite</h3>
          )}

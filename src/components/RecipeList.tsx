...
const RecipeList: React.FC<RecipeListProps> = ({ recipes }) => {
  const [filteredRecipes, setFilteredRecipes] =
    useState<RecipeWithDetails[]>(recipes);
  ...
  const handleTagSelect = (tag: Tag) => {
    setSelectedTags((prev) => {
      const isSelected = prev.some(
        (t) => t.id === tag.id || (t.id === undefined && t.name === tag.name),
      );
      if (isSelected) {
            ...
            <Button
              ...
              variant={
                selectedTags.some(
                  (t) =>
                    t.id === tag.id ||
                    (t.id === undefined && t.name === tag.name),
                )
                  ? "secondary"
                  : "outline"
              }
        ...
        {filteredRecipes.map((recipe) => (
          <Card key={recipe.id}>
            ...
            <CardFooter className="flex justify-between">
              <Button onClick={() => viewRecipeDetail(recipe)}>
                View Recipe
              </Button>
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      the recipe and remove the data from our servers.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
